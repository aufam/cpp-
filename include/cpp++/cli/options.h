#ifndef COMTRUE_CLI_OPTION_H
#define COMTRUE_CLI_OPTION_H
...
namespace comtrue::cli::detail {
    ...
    inline std::pair<std::unordered_set<int>, std::unordered_set<std::string>> get_one_of_sets(const comtrue::cli::Option &opt) {
    ...
    template <typename T>
    void visitor_setup(const T *target, cxxopts::OptionAdder &add, const comtrue::cli::Option &opt) {
        ...
        if (!opt.one_of.empty()) {
            ...
            help = fmt::format("{}{}(value: {{{}{:?}}})", help, help.empty() ? "" : " ", fmt::join(one_of_int, ", "), fmt::join(one_of_str, ", "));
        }
    ...
    template <typename T>
    void visitor_target(T *target, const cxxopts::ParseResult &parser, const comtrue::cli::Option &opt) {
        auto [one_of_int, one_of_str] = get_one_of_sets(opt);
            ...
            if (parser.count(opt.key_str)) {
                *target = parser[opt.key_str].as<typename T::value_type>();
                if constexpr (std::is_same_v<typename T::value_type, int>) {
                    if (!opt.one_of.empty() && one_of_int.count(**target) == 0) {
                        throw comtrue::cli::parse_error(fmt::format("Assertion error: {} not in {}", **target, one_of_int));
                    }
                } else if constexpr (std::is_same_v<typename T::value_type, std::string>) {
                    if (!opt.one_of.empty() && one_of_str.count(**target) == 0) {
                        throw comtrue::cli::parse_error(fmt::format("Assertion error: {:?} not in {}", **target, one_of_str));
                    }
                }
            }
        } else {
...
inline std::vector<std::string> comtrue::cli::parse(const std::string &app_name, int argc, char **argv, const std::vector<Option> &opts) {
    try {
...
inline std::vector<std::string> comtrue::cli::parse_or_throw(const std::string &app_name, int argc, char **argv, const std::vector<Option> &opts) {
#ifdef _WIN32
    // Windows passes argv as ANSI or UTF-16; convert properly
    int       wargc = 0;
    ...
    for (int i = 0; i < wargc; ++i) {
        const std::wstring &wstr = wargv[i];
        if (wstr.empty()) {
            utf8_args.push_back("");
            continue;
        }
    ...
    // optional: set console output to UTF-8
...
namespace comtrue::cli::detail {
    ...
    template <typename E>
    struct is_optional_enum<std::optional<E>> : std::bool_constant<std::is_enum_v<E>> {};

    template <typename T>
    constexpr bool is_optional_enum_v = is_optional_enum<T>::value;

    template <typename T>
    comtrue::cli::Option make_option_from_field(Context &ctx, std::string_view spec, T &field) {
        comtrue::cli::Option opt{};
        if constexpr (std::is_enum_v<T>) {
            opt.target = &ctx.set(spec, std::string(magic_enum::enum_name(field)));
            for (std::string_view name : magic_enum::enum_names<T>())
                opt.one_of.emplace(std::string(name));
        } else if constexpr (is_optional_enum_v<T>) {
            opt.target = &ctx.set(spec, field.has_value() ? std::make_optional(std::string(magic_enum::enum_name(*field))) : std::nullopt);
            for (std::string_view name : magic_enum::enum_names<typename T::value_type>())
                opt.one_of.emplace(std::string(name));
        } else {
        ...
        for (auto &p : split(spec, ',')) {
            ...
            if (p.size() == 1 && std::isalpha(static_cast<unsigned char>(p[0]))) {
                opt.key_char = p[0];
            } else if (std::string_view help_prefix = "help="; p.size() >= help_prefix.size() && p.compare(0, help_prefix.size(), help_prefix) == 0) {
    ...
    void assign_field_from_context(Context &ctx, std::string_view spec, T &field) {
        if constexpr (std::is_enum_v<T>)
            field = magic_enum::enum_cast<T>(ctx.get<std::string>(spec)).value();
        else if constexpr (is_optional_enum_v<T>) {
            auto val = ctx.get<std::optional<std::string>>(spec);
            if (val.has_value())
                field = magic_enum::enum_cast<typename T::value_type>(*val);
            else
                field = std::nullopt;
        } else
...
namespace comtrue::cli {
    inline static constexpr const char *TagName = "opt";

    /// T must be a user-defined struct where all fields are tagged using `comtrue::Tag`
    /// @example
    /// ```c++
    /// struct User {
    ///     comtrue::Tag<std::string> name = "opt:`name,positional,help=Specify user's name`"; // positional argument
    ///     comtrue::Tag<int> age = "opt:`a,age,help=specify user's age`"; // required option with flag -a,--age
    ///     comtrue::Tag<std::optional<std::string>> email = {"opt:`email`", "user@example.com"}; // optional arg with default
    ///     value
    /// };
    /// ```
    template <typename T>
    T parse(const std::string &app_name, int argc, char **argv) {
        T                                 options = {};
        detail::Context                   ctx     = {};
        std::vector<std::string_view>     specs(boost::pfr::tuple_size_v<T>);
        std::vector<comtrue::cli::Option> opts;

        boost::pfr::for_each_field(options, [&](auto &opt, size_t i) {
            if constexpr (is_tagged_v<std::decay_t<decltype(opt)>>) {
                if (auto &tag = specs[i] = opt.get_tag(TagName); !tag.empty())
                    opts.push_back(detail::make_option_from_field(ctx, tag, opt.get_value()));
            }
        });

        comtrue::cli::parse(app_name, argc, argv, opts);

        boost::pfr::for_each_field(options, [&](auto &opt, size_t i) {
            if constexpr (is_tagged_v<std::decay_t<decltype(opt)>>) {
                if (auto &tag = specs[i]; !tag.empty())
                    detail::assign_field_from_context(ctx, tag, opt.get_value());
            }
        });
